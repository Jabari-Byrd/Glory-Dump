...
function calculateCooldown(amount, isTheft = false) {
  const minCooldown = isTheft
    ? CONSTANTS.THEFT_COOLDOWN_MIN
    : CONSTANTS.TRANSFER_COOLDOWN_MIN;
  const maxCooldown = isTheft
    ? CONSTANTS.THEFT_COOLDOWN_MAX
    : CONSTANTS.TRANSFER_COOLDOWN_MAX;
...
function isValidDumpAssignment(amount) {
  return (
    amount >= CONSTANTS.MIN_DUMP_ASSIGNMENT &&
    amount <= CONSTANTS.MAX_DUMP_ASSIGNMENT
  );
}
...
function runTests() {
  ...
  assert(
    CONSTANTS.EPOCH_DURATION === 2592000,
    "Epoch duration should be 30 days in seconds",
  );
  assert(
    CONSTANTS.WAITING_PERIOD === 604800,
    "Waiting period should be 7 days in seconds",
  );
  assert(
    CONSTANTS.TRANSFER_FEE_BASIS_POINTS === 30,
    "Transfer fee should be 0.3%",
  );
  ...
  assert(
    actualFee1 === expectedFee1,
    `Expected fee ${expectedFee1}, got ${actualFee1}`,
  );
  ...
  assert(
    actualFee2 === expectedFee2,
    `Expected fee ${expectedFee2}, got ${actualFee2}`,
  );
  ...
  assert(
    smallCooldown >= CONSTANTS.TRANSFER_COOLDOWN_MIN,
    "Small cooldown should be at least minimum",
  );
  assert(
    largeCooldown <= CONSTANTS.TRANSFER_COOLDOWN_MAX,
    "Large cooldown should not exceed maximum",
  );
  assert(
    theftCooldown >= CONSTANTS.THEFT_COOLDOWN_MIN,
    "Theft cooldown should be at least minimum",
  );

  console.log(
    `Small amount (${smallAmount}): ${smallCooldown.toFixed(2)}s cooldown`,
  );
  console.log(
    `Large amount (${largeAmount}): ${largeCooldown.toFixed(2)}s cooldown`,
  );
  ...
  assert(
    isValidDumpAssignment(CONSTANTS.MIN_DUMP_ASSIGNMENT),
    "Minimum DUMP assignment should be valid",
  );
  assert(
    isValidDumpAssignment(CONSTANTS.MAX_DUMP_ASSIGNMENT),
    "Maximum DUMP assignment should be valid",
  );
  assert(
    !isValidDumpAssignment(CONSTANTS.MIN_DUMP_ASSIGNMENT - 1),
    "Below minimum should be invalid",
  );
  assert(
    !isValidDumpAssignment(CONSTANTS.MAX_DUMP_ASSIGNMENT + 1),
    "Above maximum should be invalid",
  );

  const validAmount = 5_000_000_000; // 5B DUMP
  assert(
    isValidDumpAssignment(validAmount),
  ...
  assert(
    CONSTANTS.CRITICAL_BOUNTY > CONSTANTS.HIGH_BOUNTY,
    "Critical bounty should be higher than high",
  );
  assert(
    CONSTANTS.HIGH_BOUNTY > CONSTANTS.MEDIUM_BOUNTY,
    "High bounty should be higher than medium",
  );
  assert(
    CONSTANTS.MEDIUM_BOUNTY > CONSTANTS.LOW_BOUNTY,
    "Medium bounty should be higher than low",
  );
  ...
  assert(
    waitingPeriodEnd > epochEndTime,
    "Waiting period end should be after epoch end",
  );
  ...
  assert(
    epochDuration === CONSTANTS.EPOCH_DURATION,
    "Calculated epoch duration should match constant",
  );
  assert(
    waitingDuration === CONSTANTS.WAITING_PERIOD,
    "Calculated waiting duration should match constant",
  );
  ...
  assert(
    zeroCooldown === CONSTANTS.TRANSFER_COOLDOWN_MIN,
    "Zero amount should use minimum cooldown",
  );

  // Very large number handling
  const largeNumber = Number.MAX_SAFE_INTEGER;
  const largeFee = calculateFee(
    largeNumber,
    CONSTANTS.TRANSFER_FEE_BASIS_POINTS,
  );
